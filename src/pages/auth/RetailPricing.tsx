...
export const RetailPricing = () => {
  ...
  const getDecoratedPlan = (plan: PlanDto) => {
    const nameLower = plan.name.toLowerCase();
    if (nameLower.includes("básico")) {
      return {
        id: plan.planId,
        name: plan.name,
        price: `$${plan.price}`,
        ...
        features: [
          `Límite de ${plan.maxUsers} usuarios`,
          "Límite de 2 roles",
    ...
    if (nameLower.includes("profesional")) {
      return {
        id: plan.planId,
        name: plan.name,
        price: `$${plan.price}`,
    ...
    return {
      id: plan.planId,
      name: plan.name,
      price: plan.price >= 999 ? "A medida" : `$${plan.price}`,
  ...
  if (isLoading) {
    return (
      <div className="w-full max-w-5xl bg-white/80 backdrop-blur-xl border border-slate-200 p-8 rounded-3xl shadow-xl flex flex-col items-center justify-center min-h-[400px]">
        <Loader2 className="w-10 h-10 animate-spin text-primary mb-4" />
        <p className="text-slate-600 font-medium">Cargando planes de suscripción...</p>
      </div>
    );
  }
